...
func Post(res http.ResponseWriter, req *http.Request) {
	...
	for i, f := range files {
		...
		// attrs is file
		_, attrs, err := saveToGCS(context.Background(), file, os.Getenv("BUCKET_NAME"), fmt.Sprintf("%s%d", id, i))
		if err != nil {
...
func saveToES(ctx context.Context, post *model.Post, id string) {
	...
	// Create a client
	es_client, err := elastic.NewClient(elastic.SetURL(os.Getenv("ES_URL")), elastic.SetSniff(false))
	if err != nil {
		panic(err)
		return
	}
...
func saveToBigTable(ctx context.Context, post *model.Post, id string) {
	...
	bt_client, err := bigtable.NewClient(ctx, os.Getenv("GCP_PROJECT_ID"), os.Getenv("BT_INSTANCE"))
	if err != nil {
		fmt.Println(err)
		panic(err)
		return
